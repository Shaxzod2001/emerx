...
const { settings, pushSubscriptions } = require('../database');
...
async function sendToAllExcept(excludeUserId, payload) {
  await init();
  const subs = await pushSubscriptions.findAsync({});
  const active = subs.filter(s => s.active !== false && s.userId !== excludeUserId);
  await Promise.all(active.map(s => sendToSubscription(s, payload)));
}

module.exports = { getPublicKey, sendToUser, sendToAllExcept };
